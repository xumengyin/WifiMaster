...
import static com.jerry.wifimaster.wifiutils.utils.SSIDUtils.convertToQuotedString;
import static com.jerry.wifimaster.wifiutils.utils.VersionUtils.isAndroidQOrLater;
import static com.jerry.wifimaster.wifiutils.utils.VersionUtils.isJellyBeanOrLater;
import static com.jerry.wifimaster.wifiutils.utils.VersionUtils.isLollipopOrLater;
import static com.jerry.wifimaster.wifiutils.utils.VersionUtils.isMarshmallowOrLater;
...
public final class ConnectorUtils {
    ...
    public static boolean isAlreadyConnected(@Nullable WifiManager wifiManager, @Nullable String bssid, String ssid) {
        ssid=ssid.replace("\"","");
        if (bssid != null && wifiManager != null) {
            if (wifiManager.getConnectionInfo() != null && wifiManager.getConnectionInfo().getBSSID() != null &&
                    ...
                    (Objects.equals(bssid, wifiManager.getConnectionInfo().getBSSID().replace("\"","")) || Objects.equals(ssid, wifiManager.getConnectionInfo().getSSID().replace("\"","")))) {
                wifiLog("Already connected to: " + wifiManager.getConnectionInfo().getSSID() + "  BSSID: " + wifiManager.getConnectionInfo().getBSSID());
                return true;
            } else {
                if (wifiManager.getConnectionInfo() != null) {
                    wifiLog("fuck: No--bssid:" + bssid + "SSID:"+ssid+"------manager bssid" + wifiManager.getConnectionInfo().getBSSID()+"--ssid:"+wifiManager.getConnectionInfo().getSSID().replace("\"",""));
                }
                wifiLog("isAlreadyConnected: No--");
            }
        }
        return false;
    }
...
//                .addCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET)
        ...
        final ConnectivityManager.NetworkCallback networkCallback = new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(@NonNull Network network) {
                ...
                handler.postDelayed(() -> {
                    if (isAlreadyConnected(wifiManager, of(scanResult).next(scanResult1 -> scanResult1.BSSID).get(),of(scanResult).next(scanResult1 -> scanResult1.SSID).get())) {
...
//        // cleanup previous connections just in case
        ...
        final ConnectivityManager.NetworkCallback networkCallback = new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(@NonNull Network network) {
                ...
                // read more here: https://github.com/ThanosFisherman/WifiUtils/issues/63.
                handler.postDelayed(() -> {
                    if (isAlreadyConnected(wifiManager, ssid,ssid)) {
                        wifiConnectionCallback.successfulConnect();
                    } else {
                        wifiConnectionCallback.errorConnect(ConnectionErrorCode.ANDROID_10_IMMEDIATELY_DROPPED_CONNECTION);
                    }
